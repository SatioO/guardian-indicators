var __pineMod = (() => {
  ...
  function __gScriptRunV2(__gScriptInternalContextV1) {
    ...
    const [sumPriceVolume, sumVolume] = ta.scan([0, 0], (prev, bar2, i) => {
      const fresh = i === 0 || dayKeys[i] !== dayKeys[i - 1];
      const priceVolume = (fresh ? 0 : prev[0]) + src.get(i) * bar2.volume;
      const vol = (fresh ? 0 : prev[1]) + bar2.volume;
...
globalThis.compute = globalThis.__pine.makeComputeForVersion(2,"g-script-v2",__pineMod.__gScriptRunV2,{"mode":"native-event","declarationFingerprint":"pine-event-declarations/v1/7cfbcd9302840ad9","declarations":[{"slot":0,"kind":"plot","kindOrdinal":0,"source":{"file":"indicator.ts","line":26,"column":1},"valueType":"number","qualifiers":{"value":"series","title":"const","color":"input","lineWidth":"input","style":"const","histBase":"const","editable":"const","showLast":"const","display":"const","format":"const","precision":"const","offsetBars":"const","forceOverlay":"const"},"style":"line","histBase":0}],"alertConditions":[{"id":"cross-above","title":"Close crossed above VWAP","message":"The close crossed above VWAP"},{"id":"cross-below","title":"Close crossed below VWAP","message":"The close crossed below VWAP"}]});
